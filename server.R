...
library(shiny)
 ...
 # 1. first we use a FourGram; the first three words of which are the last three words of the user ovided sentence 
 #    for which we are trying to predict the next word. The FourGram is already sorted from highest to west frequency 
 # 2. If no FourGram is found, we back off to ThreeGram (first two words of ThreeGram last two words the sentence) 
 # 3. If no FourGram is found, we back off to TwoGram (first word of TwoGram last word of the ntence) 
 # 4. If no TwoGram is found, we back off to OneGram (the most common word with highest quency) 
 # 
 ...
 { 
   ...
   # 2. Next test the Three Gram using the three gram data frame 
     ...
     { 
         ...
         mesg <- "No word found, hence the most frequent word is suggested." 
     } 
